            ...
            <td colspan="9">
                <table cellpadding="0" cellspacing="0" class="tablePadding tableBorder" align="left" style="font-size: 12px;font-family: tahoma">
                    ...
                    foreach($pr->Details as $idx => $detail) {
                        ...
                        $dtx = addslashes($detail->Id.'|'.$detail->ItemId.'|'.$detail->ItemCode.'|'.$detail->PartNo.'|'.$detail->ItemName.'|'.$detail->UomCd.'|'.$detail->Qty.'|'.$detail->MrDetailId.'|'.$detail->MrNo);
                    ...
                    <tr>
                        ...
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <?php if ($acl->CheckUserAccess("inventory.pr", "add")) { ?>
        ...
        <table cellpadding="0" cellspacing="0" class="tablePadding" style="font-size: 12px;font-family: tahoma">
            ...
            <tr>
                ...
                <td><input type="text"  class="easyui-textbox" id="aUomCd" name="aUomCd" style="width:100px" value="" readonly/>
                    <input type="hidden" name="xPrQty" id="xPrQty" value="0">
...
<script type="text/javascript">
    $( function() {
        ...
        $('#ProjectId').combobox({
            onChange: function(pri){
                urz = "<?php print($helper->site_url("inventory.pr/getjson_mritems/"));?>"+pri;
        ...
        $('#aItemSearch').combogrid({
            panelWidth:800,
            url: "<?php print($helper->site_url("inventory.pr/getjson_mritems/".$pr->ProjectId));?>",
            idField: 'id',
            textField: 'item_code',
            mode: 'remote',
            fitColumns: true,
    ...
    function fEditDetail(dta){
        ...
        if (PrStatus == 1) {
            $('#dlg').dialog('open').dialog('setTitle', 'Edit P/R Item - Ex. MR No. '+dtx[8]);
            $('#fm').form('clear');
            $('#aMode').val('E');
            $('#aId').val(dtx[0]);
            $('#aItemId').val(dtx[1]);
            ...
            $('#aPrQty').textbox('setValue',dtx[6]);
            $('#xPrQty').val(dtx[6]);
            $('#aMrDetailId').val(dtx[7]);
    ...
    function fDelDetail(dId) {
        var PrStatus = "<?php print($pr->StatusCode);?>";
        if (PrStatus == 1) {
            var urx = "<?php print($helper->site_url("inventory.pr/delete_detail/")); ?>" + dId;
    ...
    function saveDetail(){
        ...
        if (dIti > 0 && dQty > 0){
            if (tMod == 'N') {
                        ...
                        if (PrId > 0) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    var rst = data.split('|');
                                    if (rst[0] == 'OK') {
                                        location.href = "<?php print($helper->site_url("inventory.pr/add/")); ?>" + PrId;
                                    } else {
                                        alert(data);
                                    }
                                }
                ...
                var aid = $("#aId").val();
                var urd = "<?php print($helper->site_url("inventory.pr/edit_detail/")); ?>" + aid;
                $.ajax({
                    type: 'POST',
                    url: urd,
                    data: {aPrId: PrId, aId: aid, aItemId: dIti, aPrQty: dQty, aUomCd: dUom, aMrDetailId: dMdi, xPrQty: xQty,},
                    success: function (data) {
                        var rst = data.split('|');
                        if (rst[0] == 'OK') {
                            location.href = "<?php print($helper->site_url("inventory.pr/add/")); ?>" + PrId;
