...
?>
<br />
<div id="p" class="easyui-panel" title="G/N Entry" style="width:100%;height:100%;padding:10px;" data-options="footer:'#ft'">
    <table cellpadding="0" cellspacing="0" class="tablePadding" align="left" style="font-size: 13px;font-family: tahoma">
        <tr>
            ...
            <td><input type="text" class="f1 easyui-textbox" style="width: 130px" id="DocumentNo" name="DocumentNo" value="<?php print($gn->DocumentNo != null ? $gn->DocumentNo : '[AUTO]'); ?>" readonly/></td>
        ...
        <tr>
            ...
            <td><input type="text" class="easyui-datebox" style="width: 130px" id="GnDate" name="GnDate" data-options="formatter:myformatter,parser:myparser" required="required" value="<?php print($gn->FormatDate(SQL_DATEONLY));?>"/></td>
            <td class="right">Invoice No :</td>
            <td><input type="text" class="f1 easyui-textbox" style="width: 130px" id="InvoiceNo" name="InvoiceNo" value="<?php print($gn->InvoiceNo); ?>" readonly/></td>
        </tr>
        <tr>
            <td class="right">Notes :</td>
            <td><input type="text" class="easyui-textbox" name="Note" id="Note" data-options="multiline:true" style="width: 250px; height: 40px;" value="<?php print($gn->Note);?>"></td>
        </tr>
        <tr>
            <td>&nbsp;</td>
        </tr>
            ...
            <td colspan="9">
                <table cellpadding="0" cellspacing="0" class="tablePadding tableBorder" align="left" style="font-size: 12px;font-family: tahoma">
                    ...
                    <tr>
                        ...
                        <?php if ($acl->CheckUserAccess("inventory.gn", "add")) { ?>
    ...
    <form id="fm" method="post" novalidate>
        <table cellpadding="0" cellspacing="0" class="tablePadding" style="font-size: 12px;font-family: tahoma">
            ...
            <tr>
                ...
                <td><input type="text" class="easyui-textbox right" id="aGnQty" name="aGnQty" style="width:80px" value="0" required/></td>
                ...
                <td><input type="text"  class="easyui-textbox" id="aUomCd" name="aUomCd" style="width:100px" value="" readonly/>
                    ...
                    <input type="hidden" name="aPoDetailId" id="aPoDetailId" value="0">
...
<script type="text/javascript">
    $( function() {
        ...
        $('#ProjectId').combobox({
            onChange: function(pri){
                ...
                urz = "<?php print($helper->site_url("inventory.gn/getjson_poitems/"));?>"+pri+'/'+SupplierId;
        ...
        $('#SupplierId').combobox({
            onChange: function(spi){
                ...
                urz = "<?php print($helper->site_url("inventory.gn/getjson_poitems/"));?>"+ProjectId+'/'+spi;
                $('#aItemSearch').combogrid({url: urz});
            }
        });
        ...
        $('#aItemSearch').combogrid({
            panelWidth:800,
            url: "<?php print($helper->site_url("inventory.gn/getjson_poitems/".$gn->ProjectId.'/'.$gn->SupplierId));?>",
            idField: 'id',
            textField: 'item_code',
            mode: 'remote',
            fitColumns: true,
            columns: [[
                ...
                {field: 'po_qty', title: 'QTY', width: 50},
                {field: 'po_date', title: 'PO Date', width: 80},
                {field: 'po_no', title: 'PO Number', width: 170}
            ]],
    ...
    function newItem(mid){
        if (checkMaster()) {
            $('#dlg').dialog('open').dialog('setTitle', 'Add New G/N Item');
            $('#fm').form('clear');
            $('#aMode').val('N');
            $('#aItemSearch').focus();
        }else{
    ...
    function fDelDetail(dId) {
        var GnStatus = "<?php print($gn->StatusCode);?>";
        if (GnStatus == 1) {
            var urx = "<?php print($helper->site_url("inventory.gn/delete_detail/")); ?>" + dId;
            if (confirm('Hapus Item G/N ini ?')) {
    ...
    function checkMaster() {
        ...
        var mGdt = $("#GnDate").datebox('getValue');
    ...
    function saveDetail(){
        ...
        var dQty = $("#aGnQty").textbox('getValue');
        var dUom = $("#aUomCd").textbox('getValue');
        var dPrc = $("#aPrice").val();
        var dMdi = $("#aPoDetailId").val();
        if (dIti > 0 && dQty > 0){
            var urm = "<?php print($helper->site_url("inventory.gn/proses_master/")); ?>" + GnId;
            //proses simpan dan update master
            $.post(urm,{ProjectId: mPri, SupplierId: mSpi,Note: mNot, GnDate: mGdt, GnNo: mGno}, function( data ) {
                var rst = data.split('|');
                if (rst[0] == 'OK') {
                    GnId = rst[2];
                    if (GnId > 0) {
                        ...
                        var aid = $("#aId").val();
                        if (tMod == 'N') {
                            var urd = "<?php print($helper->site_url("inventory.gn/add_detail/")); ?>" + GnId;
                        } else {
                            var urd = "<?php print($helper->site_url("inventory.gn/edit_detail/")); ?>" + aid;
                        }
                        $.ajax({
                            type : 'POST',
                            url : urd,
                            data: {aId: aid,aItemId: dIti, aGnQty: dQty, aUomCd: dUom, aPoDetailId: dMdi, aPrice: dPrc},
                            success:function (data) {
                                var rst = data.split('|');
                                if (rst[0] == 'OK') {
                                    location.href = "<?php print($helper->site_url("inventory.gn/add/")); ?>" + GnId;
                                }else {
                                    alert(data);
                                }
                            }
