...
$excelTemplate = $reader->load(APPS . "templates/rekap.dokumen.mr.xls");
...
while($rs = $report->fetch_assoc()) {
    ...
    $sheet->setCellValue('C'.$row, $rs["dept_name"]);
    $sheet->setCellValue('D'.$row, $rs["doc_no"]);
    $sheet->setCellValue('E'.$row, date('d M Y', strtotime($rs["mr_date"])));
    $sheet->setCellValue('F'.$row, $rs["status_name"]);
    $sheet->setCellValue('G'.$row, $updated);
}

header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="rekap_dokumen_mr.xls"');
