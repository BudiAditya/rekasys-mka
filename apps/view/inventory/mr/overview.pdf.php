...
$pdf->SetColumns(
    array("No", "Company", "Departemen", "No.Dokumen", "Tgl.MR", "Status", "Tgl.Update"),
    array(11, 15, 40, 50, 30, 30, 30)
);
...
$pdf->Cell(290,7,'REKAPITULASI DOKUMEN MR',0,0,'L');
$pdf->Ln();
...
while($rs = $report->fetch_assoc()) {
    ...
    $pdf->RowData(array($i, $rs["entity"], $rs["dept_name"], $rs["doc_no"], date('d M Y', strtotime($rs["mr_date"])), $rs["status_name"], $updated),
                  5, null, 0,  array("C", "L", "L", "L", "L", "L", "L"));
}
$pdf->Output("Rekap dokumen MR.pdf", "D");