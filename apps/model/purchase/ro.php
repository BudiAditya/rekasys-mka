<?php
require_once("ro_detail.php");

class Ro extends EntityBase {
	public $Id;
	public $IsDeleted;
	public $EntityId;
	public $ProjectId;
	...
	public function FillProperties(array $row) {
		...
		$this->Date = strtotime($row["ro_date"]);
	...
	public function FormatExpectedDate($format = HUMAN_DATE) {
		return is_int($this->ExpectedDate) ? date($format, $this->ExpectedDate) : date($format);
	}
	...
	public function LoadDetails($orderBy = "a.item_code") {
		...
		$detail = new RoDetail();
		$this->Details = $detail->LoadByRoId($this->Id, $orderBy);
		return $this->Details;
	}
...
FROM ic_link_pr_po AS a
	JOIN ic_rr_master AS b ON a.pr_id = b.id
...
WHERE a.id = ?id";
	 ...
	 * @return Ro[]
	 */
	public function LoadUnfinishedRo($supplierId,$entityId = 0,$projectId = 0) {
	    $sqx = "SELECT a.* FROM ic_ro_master AS a WHERE a.supplier_id = ?id AND a.status IN(3, 4, 5)";
	    if ($entityId > 0){
	...
	public function Insert() {
		$this->connector->CommandText =
"INSERT INTO ic_ro_master(payment_terms,project_id, entity_id, doc_no, ro_date, supplier_id, is_vat, is_inc_vat, expected_date, status, createby_id, create_time, notes, parent_po)
VALUES(?payment_terms,?project_id,?sbu, ?docNo, ?date, ?supplier, ?vat, ?incVat, ?eta, ?status, ?user, NOW(), ?note, ?parentRo)";
		...
		$this->connector->AddParameter("?parentRo", $this->ParentRoId);
...
WHERE id = ?id";
    ...
    public function GetJSonUnfinishedRrItems($projectId = 0, $supplierId = 0, $filter = null,$sort = 'c.item_code',$order = 'ASC') {
        $sql = "SELECT a.id,a.item_id,c.item_code,c.part_no,c.item_name,a.uom_cd,a.qty - a.po_qty as pr_qty,a.price_1 as price,b.doc_no as pr_no,b.rr_date,a.supplier_id_1 as supplier_id";
        $sql.= " FROM ic_rr_detail as a Join ic_rr_master as b On a.rr_master_id = b.id";
        $sql.= " JOIN vw_ic_item_master AS c ON a.item_id = c.id Where b.is_deleted = 0 And b.status = 3 And (a.qty - a.po_qty > 0)";
        if ($projectId > 0){
