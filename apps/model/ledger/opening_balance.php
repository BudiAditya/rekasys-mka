...
WHERE b.acc_no = ?accNo And a.entity_id = ?entityId AND year(a.bal_date) = ?year";
	...
	public function Insert() {
		$this->connector->CommandText =
"INSERT INTO ac_opening_balance1(entity_id,acc_id, bal_date, bal_debit_amt, bal_credit_amt, createby_id, create_time)
VALUES(?entity_id,?acc, ?date, ?debit, ?credit, ?user, NOW())";
	...
	public function Update($id) {
		$this->connector->CommandText =
"UPDATE ac_opening_balance1 SET
	acc_id = ?acc
	, bal_date = ?date
	, bal_debit_amt = ?debit
...
WHERE id = ?id";
	...
	public function Delete($id) {
		$this->connector->CommandText = "DELETE FROM ac_opening_balance1 WHERE id = ?id";
		$this->connector->AddParameter("?id", $id);
		return $this->connector->ExecuteNonQuery();
	}
}
