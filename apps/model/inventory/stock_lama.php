...
class Stock extends EntityBase {
	...
	// Special query
	const QUERY_STOCK_BY_SBU =
"SELECT a.item_id, SUM(CASE WHEN a.stock_type < 100 THEN a.qty ELSE a.qty * -1 END) AS qty_stock, a.uom_cd
FROM ic_stock AS a
WHERE a.is_deleted = 0 AND a.date < ?date AND a.warehouse_id IN (
	SELECT id FROM ic_warehouse WHERE entity_id = ?sbu
)
GROUP BY a.item_id, a.uom_cd";

	const QUERY_STOCK_BY_WAREHOUSE =
"SELECT a.item_id, SUM(CASE WHEN a.stock_type < 100 THEN a.qty ELSE a.qty * -1 END) AS qty_stock, a.uom_cd
FROM ic_stock AS a
WHERE a.is_deleted = 0 AND a.date < ?date AND a.warehouse_id = ?warehouseId
GROUP BY a.item_id, a.uom_cd";
	 ...
	 * @return array
	 */
	public function LoadStocksFifo($itemId, $uomCode, $warehouseId = null) {
		if ($warehouseId == null) {
...
"SELECT a.*
FROM ic_stock AS a
WHERE a.is_deleted = 0 AND a.warehouse_id = ?warehouseId AND a.item_id = ?itemId AND uom_cd = ?uomCd AND qty_balance > 0
ORDER BY a.date ASC";
	 ...
	 * @return array
	 */
	public function LoadHistoriesBetween($itemId, $uomCode, $startDate, $endDate, $warehouseId = null) {
		if ($warehouseId == null) {
			// Walau pada query kita tidak memberikan filter gudang tetapi kita harus ingat bahwa barang sudah di lock per SBU
			// Jadi jika barang 'A' di lock pada SBU 'Mall' maka jika kita filter hanya berdasarkan barang A tidak mungkin muncul di gudang lain selain gudang Mall
			// Kecuali ada yang tembak data barang 'A' ke gudang SBU lain yang mana secara system tidak mungkin terjadi
...
"SELECT a.*
FROM ic_stock AS a
WHERE a.is_deleted = 0 AND a.item_id = ?itemId AND uom_cd = ?uomCd AND a.date BETWEEN ?start AND ?end
ORDER BY a.date ASC";
		} else {
			$query =
"SELECT a.*
FROM ic_stock AS a
WHERE a.is_deleted = 0 AND a.warehouse_id = ?warehouseId AND a.item_id = ?itemId AND uom_cd = ?uomCd AND a.date BETWEEN ?start AND ?end
ORDER BY a.date ASC";
			$this->connector->AddParameter("?warehouseId", $warehouseId);
		}
