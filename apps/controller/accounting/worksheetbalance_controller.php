...
class WorksheetBalanceController extends AppController {
    ...
    public function recap() {
        ...
        if (count($this->postData) > 0) {
            ...
            $rekapMethod = $this->GetPostValue("rekapMethod");
            $isIncOb = $this->GetPostValue("isIncOb");
            ...
            $sql.= " AND a.is_deleted = 0 AND a.voucher_date BETWEEN ?start AND ?end AND b.acc_debit_id IN (SELECT id FROM acc_id) GROUP BY b.acc_debit_id;";
            $this->connector->CommandText = $sql;
            $this->connector->ExecuteNonQuery();
            ...
            $sql.= " AND a.is_deleted = 0 AND a.voucher_date BETWEEN ?start AND ?end AND b.acc_credit_id IN (SELECT id FROM acc_id) GROUP BY b.acc_credit_id;";
            $this->connector->CommandText = $sql;
            $this->connector->ExecuteNonQuery();
            ...
            if ($rekapMethod == 1 && $month > 1) {
                ...
                $sql = "CREATE TEMPORARY TABLE sum_debit_prev AS SELECT b.acc_debit_id, SUM(b.amount) AS total_debit_prev FROM ac_voucher_master AS a JOIN ac_voucher_detail AS b ON a.id = b.voucher_master_id WHERE a.status = 4";
                if ($projectId > 0){
                    $sql.= " And b.project_id = ".$projectId;
                }
                $sql.= " AND a.is_deleted = 0 AND a.voucher_date BETWEEN ?firstJan AND ?prev AND b.acc_debit_id IN (SELECT id FROM acc_id) GROUP BY b.acc_debit_id;";
                $this->connector->CommandText =
                $this->connector->ExecuteNonQuery();
                // #05: Ambil data bulan-bulan sebelumnya (kredit)
                $sql = "CREATE TEMPORARY TABLE sum_credit_prev AS SELECT b.acc_credit_id, SUM(b.amount) AS total_credit_prev FROM ac_voucher_master AS a JOIN ac_voucher_detail AS b ON a.id = b.voucher_master_id WHERE a.status = 4";
                if ($projectId > 0){
                    $sql.= " And b.project_id = ".$projectId;
                }
                $sql.= " AND a.is_deleted = 0 AND a.voucher_date BETWEEN ?firstJan AND ?prev AND b.acc_credit_id IN (SELECT id FROM acc_id) GROUP BY b.acc_credit_id;";
                $this->connector->CommandText = $sql;
                $this->connector->ExecuteNonQuery();
                ...
                $this->connector->CommandText =
                    ...
                    FROM acc_id AS a
                        ...
                        LEFT JOIN ac_opening_balance2 AS f ON a.id = acc_id AND bal_date = ?firstJan
                    ORDER BY a.acc_no";
            } else {
