...
class ReportController extends \AppController {
	...
	public function journal() {
		...
		if (count($this->getData) > 0) {
			...
			if (count($docIds) == 0) {
				...
				redirect_url("ledger.report/journal");
				return;
			}
			...
			if ($showAdditionalColumn) {
                ...
                WHERE  a.rstatus = 1 And a.is_deleted = 0 AND a.voucher_date BETWEEN ?start AND ?end AND a.doc_type_id IN ?docTypes %s";
                if ($projectId > 0){
	...
	public function recap() {
		...
		if (count($this->getData) > 0) {
			...
			if (count($docIds) == 0) {
				$this->persistence->SaveState("error", "Mohon pilih jenis dokumen terlebih dahulu. Sekurang-kurangnya pilih 1.");
				redirect_url("ledger.report/recap");
				return;
			}
...
FROM ac_voucher_master AS a
	JOIN ac_voucher_detail AS b ON a.id = b.voucher_master_id
WHERE [status] [entity] [project_id]  a.rstatus = 1 And a.is_deleted = 0 AND a.voucher_date BETWEEN ?start AND ?end AND a.doc_type_id IN ?docIds
...
FROM ac_voucher_master AS a
	JOIN ac_voucher_detail AS b ON a.id = b.voucher_master_id
WHERE [status] [entity] [project_id]  a.rstatus = 1 And a.is_deleted = 0 AND a.voucher_date BETWEEN ?firstJan AND ?prev AND a.doc_type_id IN ?docIds
GROUP BY b.acc_debit_id;";
                if ($projectId > 0){
                    $query = str_replace("[project_id]", "b.project_id = ?projectId AND", $query);
                }else{
