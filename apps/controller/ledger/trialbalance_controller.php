...
WHERE a.status = 4 AND a.rstatus = 1 And a.is_deleted = 0 AND a.voucher_date BETWEEN ?start AND ?end
...
FROM ac_voucher_master AS a
	JOIN ac_voucher_detail AS b ON a.id = b.voucher_master_id
WHERE a.status = 4 AND a.rstatus = 1 And a.is_deleted = 0 AND a.voucher_date BETWEEN ?firstJan AND ?prev
	AND b.acc_debit_id IN (SELECT id FROM acc_id)
GROUP BY b.acc_debit_id;";
				$this->connector->ExecuteNonQuery();
